...
function genererFormule($formule,$donnees,$NombreDonnee) {
  ...
  foreach ($TableauFormule as $indiceTableauFormule=>$valeur) {
    ...
    if ($operateur != Null) {
        // Récupere l'indice des opérateurs pour savoir où il se trouve dans le tableauFormule
      array_push($tableauIndiceOperateur,$indiceTableauFormule);
    ...
    foreach($TableauFormule as $indiceTableauFormule=>$valeur) {     //Parcours le tableau formule
      ...
      if($premierPassage == 1) {
        ...
        if(!preg_match('#^[0-9,\s\.-]+$#', $valeur) && is_string($valeur)) {   //Vérifie si c'est une chaine de caractère
          ...
          // Parcours le tableau avec les données

            if ($TableauFormule[$indiceTableauFormule] == $donneesColonne[0] && $donneesColonne[$i+1] != Null) {
            // Cherche le nom perso de la colonne pour ensuite rentrer les données

              $TableauResultatFormule[$i] = $donneesColonne[$i+1];
        ...
        } elseif (isset($TableauFormule[$indiceTableauFormule+1])) {
        //Vérifie si il reste un opérateur

          $TableauResultatFormule[$i] = $TableauFormule[$indiceTableauFormule];
          //Si ce n'est pas une chaine de caractère, enregistre simplement la constante
        } # fin si preg_match
      ...
      $operateur = connaitreOperateur($valeur);
      //Permet de savoir si c'est un opérateur

      if ($TableauResultatFormule[$i] != Null) {
        if ($operateur != Null && !is_null($TableauResultatFormule[$i]) && is_numeric($TableauFormule[$indiceTableauFormule+1]) ) {
        //Vérifie si c'est un opérateur et si la case suivante est une constante
        ...
        } elseif ($operateur != Null && !is_null($TableauFormule[$indiceTableauFormule+1]))  {
        //Si la case suivante n'est pas une constante alors cherche la donnée de la colonne correspondante
