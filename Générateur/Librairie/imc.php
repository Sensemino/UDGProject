...
function imc($donnees,$nbligne,$lignemat) {

// Calcul de l'imc selon la formule (poids / taille^2)
// on suppose la taille en cm et le poids en kg ==> multiplication par pow(10,4) en fin de calcul
  ...
  for ($i = 0;$i<$lignemat;$i++) {
    ...
    for($j = 0;$j<=$nbligne;$j++) {
      if($donnees[$i][$indexPoids] == 'Poids' && is_numeric($donnees[$i][$j])) {
          $donneesPoids[$j-($indexPoids+1)] = $donnees[$i][$j];
          //On ajoute 1 à $indexPoids car un tableau commence à l'indice 0 alors que nous on veut le nombre de cases
      } # fin si
  ...
  for ($i = 0; $i<$nbligne-1; $i++) {
    if ($donneesPoids[$i] == 'NULL' || $donneesTailles[$i] == 'NULL') {
      ...
      if ($donneesPoids[$i] > 0 && $donneesTailles[$i] > 0) {
        $resultatIMC[$i] = round( pow(10,4)*$donneesPoids[$i]/($donneesTailles[$i]*$donneesTailles[$i]) , 1 ) ;
      } else {
        $resultatIMC[$i] = 0 ;
      } ; # finsi
    } ; # fin si
